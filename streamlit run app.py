...
# Output: Admissions_Dashboard_Standalone.html
# ==========================================================
import os, re, time
...
pn.extension("plotly")  # enables Plotly panes inside Panel

# -------------------------
# SETTINGS
# -------------------------
FILE_PATH = r"C:\Users\manalik\Desktop\Admissions_Data.xlsx"   # <-- change if needed
OUTPUT    = os.path.join(os.path.expanduser("~"), "Desktop", "Admissions_Dashboard_Standalone.html")
DEFAULT_REGION_FALLBACK = "District"   # used when RLC col not present
...
def compute_region_targets(frame: pd.DataFrame, year: int, region_col: str) -> pd.DataFrame:
    ...
    prev2 = prev_years[-2:]  # last two historic years
    hist = (
...
# -------------------------
# Widgets (slicers) — these work in saved HTML too
# -------------------------
...
def kpis_and_insights(year, region_choice):
    ...
    # Insights (better contrast & styling)
    insights = []
...
def tables(year, region_choice):
    ...
    # Conditional coloring columns
    def _status_color(v):
        return "background-color: #d1ffd1" if v == "Achieved" else "background-color: #ffd6d6"
    styled = (
...
tmpl = pn.template.FastListTemplate(
    ...
    theme="default",  # "dark" also available
)
tmpl.config.raw_css.append("""
/* improve contrast for insights card in dark editors */
...
# -------------------------
tmpl.save(OUTPUT, embed=True, resources="inline")  # single portable file
print("✅ Saved:", OUTPUT)